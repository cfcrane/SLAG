...
$rscript = $ARGV[2]; #ZeaferredoxinsSRcap3graphics4.R
$pdffile = $ARGV[3]; #ZeaferredoxinsSRcap3graph4.pdf
$pdfheight = $ARGV[4]; #3
$pdfwidth = $ARGV[5]; #4
...
$ubound = $rmatchinglongests[0];
for ($i = 1; $i < scalar(@rmatchinglongests); $i++) {
  if ($rmatchinglongests[$i] > $ubound) {$ubound = $rmatchinglongests[$i];}
}
