...
#    close (FSQ);
    ...
    print LOG "$spadesexe -t $nthreads --phred-offset $phredoffset -o $spadesworkdir -1 $forwardextractedsfastq -2 $reverseextractedsfastq\n";
    `$spadesexe -t $nthreads --phred-offset $phredoffset -o $spadesworkdir -1 $forwardextractedsfastq -2 $reverseextractedsfastq`;
  ...
  if ($useracon == 0) {$seqfile = $tggfile;}
    ...
    for ($i = 0; $i < $nrounds; $i++) {
      ...
      print LOG "$blastdir/blastn -query $polfile -db $blastdb -out $blastout -evalue $evalue\n";
